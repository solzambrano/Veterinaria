...
import {
  ...
  Subparagraph,
  Icons,
  ContainerMoreQuestion,
  SectionTitle,
} from "./frequently-questions.style";
...
const FrequentlyQuestions = ({ category }) => {
  ...
  const filteredFaqs =
    category ? faqs.filter((section) => section.slug === category) : faqs; // si no hay slug, muestra todas las secciones
  ...
  return (
    <>
      <Container>
        ...
        <Subparagraph>
          Las dudas más comunes de nuestros clientes, resueltas
        </Subparagraph>
      </Container>
      {filteredFaqs.map((section) => {
        const frequentlyQuestions = section.questions
          .slice(0, visible)
          .map(({ question, answer, items }) => ({
            title: question,
            subtitle: answer,
            items,
          }));
      ...
      {visibilityButton && (
        <ContainerMoreQuestion onClick={moreQuestion}>
          ...
          <Subparagraph>Cargar más preguntas</Subparagraph>
        </ContainerMoreQuestion>
      )}
    </>
  );
